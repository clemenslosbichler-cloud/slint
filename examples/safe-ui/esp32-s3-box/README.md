...
- **Display** - the 320x240 ILI9342C panel on SPI2, through `mipidsi`.
  The scene positions its telltales in absolute pixels for a 640x480 window,
  and the Slint SC subset has no division to express that relative to the window
  size. The panel is exactly half of that in each direction, so the backend
  renders the scene at its design size into a PSRAM frame buffer and averages
  each 2x2 block down to one panel pixel.
- **Touch** - the GT911 controller on I2C0, polled every 20 ms.
  Its interrupt line is the pin that selects the I2C address at reset, which is
  why the vendor's driver polls it too.
- **Waiting** - `embassy_time::Timer`, so `app_main` sleeps until the next color
  step instead of spinning.
...
- The Slint SC compiler, built once into the shared target directory:

  ```
  cargo build -p slint-compiler --no-default-features --features slint-sc
  ```

  `.cargo/config.toml` points the app's `build.rs` at it, because a cross build
  doesn't share the host build directory. Override `SLINT_COMPILER` to use a
